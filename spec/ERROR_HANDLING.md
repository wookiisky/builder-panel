...
panel 窗口状态恢复、内容尺寸自适应、置顶偏好应用、监听或保存可能失败。
...
panel 窗口状态恢复、内容尺寸自适应、监听或保存失败时返回前端可展示错误，不写 session 状态。

单次 panel 内容尺寸调整失败时，当前请求结束但不缓存失败目标；后续内容、窗口或 overlay 变化仍可重新请求。

panel 自适应 controller 释放时取消尚未执行的动画帧、尺寸观察和窗口监听；已经释放的 controller 不开始新的 resize 副作用。

前端运行时缺少 `ResizeObserver` 时，窗口内容尺寸自适应降级为首屏和窗口 resize 触发，不写错误状态。
...
panel 窗口状态恢复、内容尺寸自适应或置顶偏好应用失败时，前端继续使用当前窗口展示。

当前显示器不可识别时，panel 高度只降级使用用户配置上限，不因缺少显示器工作区停止内容自适应。

panel 窗口移动、宽度保存、内容尺寸自适应或置顶偏好应用失败时，不影响 session 刷新、审批和回复。
...
panel 窗口状态恢复、内容尺寸自适应、置顶偏好应用、监听或保存失败时，前端在设置状态提示中展示错误。
...
`src/api/panelWindowApi.ts` 是前端 panel 窗口状态恢复、内容尺寸自适应和保存错误入口。
