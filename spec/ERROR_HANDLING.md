...
Codex APP schema 不可用时，不展示 Codex APP 结构化控制能力。
...
hook 安装 manifest 写入失败时，不应留下可被卸载流程当作成功安装使用的 manifest。
...
`src-tauri/src/adapters/codex_app/mod.rs` 是 Codex APP schema 探针和 notification 错误收敛入口。
...
`src/stores/panelProbeStore.test.ts` 验证默认状态。
...
`src-tauri/src/adapters/codex_app/mod.rs` 覆盖 Codex APP schema 探针和 notification 字段校验。
