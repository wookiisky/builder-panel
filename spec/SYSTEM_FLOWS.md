...
Tauri 环境读取设置后尝试恢复上次窗口位置、逻辑宽度和置顶偏好。

窗口位置、逻辑宽度和置顶偏好恢复完成或失败收敛后，前端才启动首轮内容高度自适应。

设置读取时，旧版物理像素窗口尺寸字段会被清空，不迁移为逻辑窗口宽度。

Tauri 环境监听窗口移动和尺寸变化。

窗口位置或逻辑宽度变化后，前端通过 panel 窗口状态局部保存 command 持久化位置和逻辑宽度。

窗口高度由前端根据面板内容派生并通过 Tauri 窗口 API 调整，不作为稳定设置恢复。

每轮自适应先测量独立自然内容层和当前 overlay，再从 Tauri 读取真实窗口、当前显示器工作区和缩放比例，统一转换为逻辑坐标后计算目标高度。

自然内容尺寸变化以及任意层级 overlay 的挂载、卸载都会进入同一个串行自适应请求入口。

窗口 resize 副作用串行执行；进行中收到的新请求在当前副作用完成后只按最新内容重跑一次。

设置运行期间保存成功后只应用置顶偏好，不重复恢复窗口位置或逻辑宽度。
