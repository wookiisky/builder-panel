...
主窗口默认宽度面向扩展模式工作台，默认高度面向当前内容。

应用读取设置后会尝试恢复上次窗口位置、窗口宽度和面板置顶状态。

用户关闭面板置顶设置并保存成功后，当前窗口不再保持在其它窗口上方。

窗口移动或调整宽度后，Tauri 环境会保存新的窗口位置和逻辑宽度。

窗口高度随当前 panel 内容自适应，不从历史高度恢复。

panel 内容较少时，窗口不会保留大面积空白背景。

panel 内容较少时，高度按当前可见内容计算，不沿用当前 WebView 视口高度。

用户手动拉高窗口后，窗口会重新收敛到当前内容所需高度，不保留手动增加的空白高度。

用户手动调整窗口宽度后，窗口保留新宽度，并按新换行结果重新计算高度。

设置弹窗打开时，窗口会把设置弹窗作为当前可见内容参与高度自适应。

窗口总高度默认最多自适应到 400 逻辑像素，用户可在设置中调整该最大高度。

panel 内容超过窗口最大高度后，内容区内部滚动。

多显示器或不同缩放比例环境下，窗口高度按当前显示器工作区限制。
...
panel 逻辑宽度大于 560 时，session 行来源标签和项目名同一行展示，thread 名在身份区下一行单行展示。

panel 逻辑宽度大于 560 时，状态、身份、当前输出和时间操作区保持在同一主行；常用 665 逻辑宽度不得提前切换为堆叠布局。

panel 逻辑宽度不大于 560 时，session 行保持状态、内容、时间操作三列；状态和时间操作跨两行，来源标签、项目名和 thread 名位于内容区第一行，当前输出位于内容区第二行。
...
当前输出文本已经完整展示时不展示 tooltip；存在未展示段落、字符截断或两行视觉截断时才展示 tooltip。

当前输出文本 tooltip 保留段内换行，并按 Markdown 渲染标题、列表、引用、代码、链接和基础行内强调。

当前输出文本 tooltip 在视口内展示；下方空间不足时会改为在当前输出文本上方展示。

用户双击当前输出文本 tooltip 的非链接区域时，panel 会按对应 session 行点击规则尝试跳转。

用户双击当前输出文本 tooltip 内的 Markdown 链接区域时，保留链接行为，不触发 session 跳转。
...
阶段 7 设置弹窗包含 General、Panel、Display、Agents、Hook Install、Replies、Presets、Terminal、Advanced 和 Logging。
