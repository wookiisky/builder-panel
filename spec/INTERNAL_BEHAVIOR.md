...
Panel 设置保存窗口位置和窗口逻辑宽度。

Panel 设置保存窗口最大逻辑高度，低于边界时归一化为默认值，高于边界时归一化到最大允许值。

Panel 设置中的旧版窗口尺寸字段不再作为运行时高度恢复来源。

Panel 设置归一化时会清空旧版物理像素窗口尺寸字段，不把旧尺寸迁移为逻辑窗口宽度。
...
panel 窗口移动和逻辑宽度变化通过局部保存 command 更新 Panel 设置，不覆盖其它设置分组。

panel 窗口高度由前端内容测量派生，并按当前窗口位置和屏幕可用底边约束，不写入稳定设置。

panel 高度约束必须使用 Tauri 当前窗口、当前显示器工作区和缩放比例转换后的统一逻辑坐标，不得混用浏览器 screen 坐标和 Tauri 物理坐标。

panel 首轮内容高度自适应必须等待窗口位置、逻辑宽度和置顶偏好恢复完成或失败收敛。

panel resize 必须串行执行；进行中的中间请求合并为一次最新状态重跑，禁止多个 `setSize` 并发竞争。

panel 是否需要 resize 必须比较 Tauri 返回的真实窗口高度与当前目标高度，不得用上次请求目标代替真实高度。

panel 内容高度副作用必须保留调用时读取到的真实逻辑宽度；用户逻辑宽度仍由窗口状态局部保存。

单次 panel resize 失败不得写入已确认尺寸或阻止后续内容变化重新请求。

panel 窗口内容测量不得把当前 WebView 视口高度或根布局固定高度作为自然内容高度。

panel 自然内容层和受限滚动视口必须分离；滚动上限不得反向成为自然内容高度来源。

panel 窗口内容测量必须把当前打开的 overlay 面板作为可见内容高度候选。

动态子级 overlay 的挂载和卸载必须触发重新测量，并把新挂载 overlay 注册为尺寸观察目标。
...
Tauri 窗口位置、逻辑宽度读取、内容高度自适应、监听与恢复属于前端 Tauri API 边界副作用。

panel 窗口相关 Tauri API 边界副作用必须先通过 Tauri 运行时判断入口区分 Tauri 环境和浏览器开发环境。

浏览器开发环境不得执行 Tauri 窗口位置、宽度恢复和内容高度自适应。
