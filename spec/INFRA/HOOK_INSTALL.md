...
卸载成功后会删除 manifest，避免陈旧 manifest 再次生效。

设置页提供 Codex CLI hook 和 Claude CLI hook 选择、预览、安装和卸载入口。

设置页安装前会调用预览入口展示将修改的文件、备份文件和 manifest 路径。
...
`src-tauri/src/adapters/hook_install/mod.rs` 是 hook 安装预览、安装、备份、manifest 和卸载入口。

`src-tauri/src/tauri_api/commands.rs` 是 hook 安装预览、安装和卸载 command 入口。
...
`src-tauri/src/adapters/hook_install/mod.rs` 覆盖安装预览、Codex hook 写入、重复安装替换、混合 group 保留用户 handler、重复 agent 去重、失败回滚、备份恢复、manifest 删除和缺失配置卸载删除。
