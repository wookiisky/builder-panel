...
4. 需要快速判断 agent 当前状态、失败原因和下一步动作的用户。
...
1. 首版扩展模式：对应完整控制面板，展示 session 列表、执行信息、状态动画和操作区。
2. 未来 mini 模式：只作为类型和布局扩展点预留，首版不实现、不验收。
3. 视觉风格参考 Codex 和 Claude Code 的克制、工程化、可扫读风格。
...
1. 当前不提供独立历史详情入口。
2. 当前不提供独立历史详情查询、搜索、筛选、复制或导出能力。
3. session 行只展示摘要、状态、等待交互和 follow-up 入口。

验收：

1. session 行和详情区域不展示独立历史详情入口。
2. Tauri command 和前端 API 不提供独立历史详情查询接口。
...
8. 设置页必须支持开放性回复输入框偏好：
   ...
   - 发送快捷键。
   - 最大长度。
9. 配置结构可预留默认 UI 模式字段，首版固定为扩展模式，不提供 mini 切换设置。
10. 设置页必须支持用量展示开关。
11. 设置页必须展示各 agent 用量数据来源状态。
12. 配置必须保存在本地。
13. 配置读取必须经过校验。
14. 配置写入必须采用临时文件加原子替换。
...
10. 详情区域展示审批、选项、开放性回复、快捷回复和 follow-up 入口。
...
### 4.5 Follow-up 行内输入 UI

必须包含：

1. 完成或失败 session 第一行右侧的展开按钮。
2. 展开后的快捷输入区域。
3. 展开后的单行输入框。
4. 展开后的发送按钮。
5. 发送中和发送失败状态。
...
1. 当前不提供独立历史详情能力。
2. 当前不保留独立历史详情 service、port、adapter 缓存或 Tauri command。
3. 当前不从 transcript / JSONL 反向读取历史详情数据。
...
1. session 列表必须保持捕捉顺序稳定。
2. 默认只保存轻量 UI 状态。
4. 长内容默认折叠。
5. 1000 个 session 更新和 follow-up 展开集合操作应保持可用不卡顿。
...
7. 完成或失败 session 默认单行，点击展开后才展示 follow-up 输入区。
8. 第二行快捷输入、输入框和发送按钮不撑破布局。
9. 扩展模式展示执行信息和状态动画。
...
6. 1000 个 session 更新不卡顿。
7. follow-up 展开集合操作保持轻量。
...
3. 长文本不溢出。
4. 按钮不重叠。
5. 完成或失败 session 默认不自动展开第二行。
6. 不支持能力时 UI 不误导用户。
7. 扩展模式支持回复和 follow-up 相关操作。
...
5. 支持的托管会话可在完成或失败后手动展开 follow-up 输入区。
...
3. 用户能用预设命令启动新对话。
4. 不支持回写时明确降级。
5. 用户能手动展开完成或失败 session 的 follow-up 输入区。
